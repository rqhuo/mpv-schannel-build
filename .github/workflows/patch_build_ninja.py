...
v16.3 fixes:
  - Fix rescue cp: write rescue_cp.sh script to disk and call it with
    `bash "path/rescue_cp.sh"` instead of `bash -lc '...'`.
    cmd.exe doesn't support single quotes, so bash -lc '...' was
    parsed incorrectly (|| true became || 'true'' -> command not found).
  - Script uses absolute path to avoid CWD issues.
  - Script scans all 4 known DLL locations and copies first found.
...
def make_rescue_command(step_name, build_dir):
    ...
    Using a script file avoids cmd.exe single-quote parsing issues
    that broke bash -lc '...' in v16.2."""
...
print(f"v16.3 patch_build_ninja.py: patched {patched} build rule COMMANDs -> cmake -E touch chain")
print(f"v16.3 patch_build_ninja.py: RERUN_CMAKE disabled = {rerun_patched}")
print(f"v16.3 patch_build_ninja.py: file = {NINJA_FILE}")
