...
# rescue_cp.sh - V17.2 rescue script
...
'''
    ...
    print(f"v17.2 rescue_cp.sh written to {script_path}")
    return script_path
...
while i < n:
    ...
    if build_match:
        ...
        # V17 TRACE: dump decision for every rule touching NEVER_TOUCH pkgs
        # --- begin V17 TRACE block ---
        _nt_trace_tokens = []
        for _out in outputs:
            _out_n = _out.replace('\\', '/')
            _matched_pkg = None
            _stamp_pattern = None
            for _p in NEVER_TOUCH_PKGS:
                _pat = f'/{_p}-stamp/{_p}-'
                if _pat in _out_n:
                    _matched_pkg = _p
                    _stamp_pattern = _pat
                    break
            if _matched_pkg is not None:
                _idx = _out_n.index(_stamp_pattern) + len(_stamp_pattern)
                _step = _out_n[_idx:].split('/')[0].split('\\')[0]
                _st = 'safe_skip' if _step in NEVER_TOUCH_SAFE_SKIP else 'core_step'
                _nt_trace_tokens.append(f"{_st}:{_step}")
        if _nt_trace_tokens:
            _wl = is_whitelisted_output(outputs)
            _rs = bool(is_rescue_step(outputs))
            print(f"V17.2 NEVER_TOUCH DECISION: outputs[0]={outputs[0]} -> decisions={_nt_trace_tokens} rescue={_rs} whitelist_result={_wl} -> ACTION={'ORIGINAL_COMMAND' if not _wl else 'CMAKE_E_TOUCH'}")
            del _nt_trace_tokens, _wl, _rs, _matched_pkg, _stamp_pattern, _step, _st, _idx
        # --- end V17 TRACE block ---
...
print(f"v17.2 patch_build_ninja.py: patched {patched} build rule COMMANDs -> cmake -E touch chain")
print(f"v17.2 patch_build_ninja.py: NEVER_TOUCH_PKGS = {NEVER_TOUCH_PKGS} (core steps PROTECTED)")
print(f"v17.2 patch_build_ninja.py: RERUN_CMAKE disabled = {rerun_patched}")
print(f"v17.2 patch_build_ninja.py: file = {NINJA_FILE}")
